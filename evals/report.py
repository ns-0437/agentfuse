...
def console_report(full: Metrics, rows, results: list[ScenarioResult],
    ...
    a("HEADLINE")
    a(SUB)
    a(f"  precision           {_pct(full.precision)}      (trust a trip when we see one)")
    a(f"  recall              {_pct(full.recall)}      (real failures caught)")
    a(f"  F1                  {_pct(full.f1)}")
    a(f"  false-positive rate {_pct(full.false_positive_rate)}      (healthy runs halted)")
    a(f"  attribution         {_pct(full.attribution_accuracy)}      (right detector for the right failure)")
